...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'presentation', label: 'Présentation App', icon: '📖' },
  ];
      ...
      <aside className="w-full md:w-64 bg-[#1a237e] text-white p-6 flex-shrink-0 flex flex-col shadow-2xl z-20">
        ...
        <div className="mt-8 pt-4 border-t border-blue-800 space-y-3">
          <div className="flex items-center justify-between px-2 mb-2">
            <span className="text-[9px] font-bold text-blue-300 uppercase">{user.username}</span>
            <button onClick={onLogout} className="text-[9px] font-black text-red-300 uppercase hover:text-red-100">Déconnexion</button>
          </div>
          
          <button onClick={() => db.exportFullBackup(user.id)} className="w-full bg-blue-700/50 hover:bg-blue-600 text-white text-[9px] py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all uppercase tracking-widest">
            <span>💾</span> Sauvegarde
          </button>
          
          <button onClick={() => fileInputRef.current?.click()} className="w-full bg-gray-700/50 hover:bg-gray-600 text-white text-[9px] py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all uppercase tracking-widest">
            <span>📥</span> Import
          </button>
